...
class Entity
{
  public:
    ...
    inline bool HasGroup(Group group)
    {
    ...
    void Refresh()
    {
        for (auto i(0u); i < MAX_GROUPS; ++i)
        {
            ...
            vec.erase(std::remove_if(std::begin(vec), std::end(vec),
                                     [i](Entity *entity) { return !entity->IsActivated() || !entity->HasGroup(i); }),
                      std::end(vec));
        }

        entities.erase(std::remove_if(std::begin(entities), std::end(entities),
                                      [](const std::unique_ptr<Entity> &entity) { return !entity->IsActivated(); }),
                       std::end(entities));
    }
